...
def _safe_model_snippet(text: str, limit: int = 2000) -> str:
    """Return a short debug snippet with obvious secret-shaped values redacted."""
    snippet = (text or "")[:limit]
    for pattern in _SECRET_SNIPPET_PATTERNS:
        if pattern.groups >= 3:
            snippet = pattern.sub(lambda m: f"{m.group(1)}{m.group(2)}{m.group(3)}[redacted]", snippet)
        else:
            snippet = pattern.sub(lambda m: f"{m.group(1)}[redacted]", snippet)
    return snippet
...
def _parse_coder_input_context(user: str) -> dict[str, Any]:
    """Extract enough context from the coder payload for deterministic fallback."""
    try:
    ...
    return {
        "prompt": context.get("goal") or requirements.get("summary") or payload.get("prompt") or "Create a production-ready Amarktai Builder website.",
        "brand_name": context.get("brand_name") or "Amarktai Builder",
        "audience": context.get("audience") or context.get("target_audience") or requirements.get("audience") or DEFAULT_AUDIENCE,
        "mode": context.get("mode") or payload.get("mode") or "website",
        "features": context.get("features") or requirements.get("core_features") or [],
        "sections": context.get("sections") or plan.get("sections") or [],
        "style": context.get("style") or "dark luxury glassmorphism with cinematic lighting",
    }


def _deterministic_coder_fallback_files(user: str, reason: str) -> dict[str, Any]:
    """Generate a safe static preview site when Coder output is unparsable."""
    ctx = _parse_coder_input_context(user)
    brand = escape(str(ctx["brand_name"])[:80] or "Amarktai Builder")
    audience = escape(str(ctx["audience"])[:180] or DEFAULT_AUDIENCE)
    style = escape(str(ctx["style"])[:160])
    features = [str(item) for item in ctx.get("features") or [] if str(item).strip()]
    if not features:
        features = [
            "AI software generation",
            "repo import and continuation",
            "live previews",
            "quality gates",
            "GitHub PR automation",
            "graceful media fallbacks",
        ]
    sections = [str(item) for item in ctx.get("sections") or [] if str(item).strip()]
    if not sections:
        sections = [
            "cinematic hero",
            "AI software factory overview",
            "live build pipeline",
            "repo workflow",
            "quality and deployment",
        ]
    feature_cards = "\n".join(
        f"<article><h3>{escape(item[:80])}</h3><p>Production-minded execution with clear status, previewable output, and repair loops.</p></article>"
        for item in features[:6]
    )
    timeline = "\n".join(
        f"<li><span>{idx:02d}</span><strong>{escape(item[:90])}</strong></li>"
        for idx, item in enumerate(sections[:6], start=1)
    )
    html = f"""<!doctype html>
<html lang="en">
<head>
  ...
  <title>{brand} | AI Software Factory</title>
  <link rel="stylesheet" href="styles.css">
</head>
<body>
  <header class="topbar">
    <a class="brand" href="#hero">{brand}</a>
  ...
  <main>
    <section id="hero" class="hero">
      <div class="hero-copy">
        <p class="eyebrow">Fallback preview generated by Amarktai Builder</p>
        <h1>Elite AI software builds, previews, repairs, and PRs from one production dashboard.</h1>
        <p class="lede">{brand} helps {audience} create premium websites and full-stack apps with model orchestration, repo workflows, live previews, and quality gates.</p>
        <div class="actions">
          <a class="button primary" href="#pipeline">Start the build</a>
          <a class="button secondary" href="#quality">View quality gates</a>
        </div>
      </div>
      <div class="preview" aria-label="Animated product preview">
        <div class="preview-panel large"></div>
        <div class="preview-panel mid"></div>
        <div class="preview-panel small"></div>
      </div>
    </section>
    <section id="factory" class="section">
      <p class="eyebrow">AI software factory</p>
      <h2>Built for tonight's go-live validation with graceful model-output recovery.</h2>
      <p class="section-copy">This deterministic fallback appears only when Coder returns unparsable output. It preserves preview generation and quality reporting while exposing the parser failure in build events.</p>
      <div class="cards">{feature_cards}</div>
    </section>
    <section id="pipeline" class="section split">
      <div>
        <p class="eyebrow">Pipeline</p>
        <h2>Planner, Scout, Architect, Coder, Reviewer, Quality.</h2>
        <p class="section-copy">The build continues with real files instead of failing on malformed Coder formatting.</p>
      </div>
      <ol class="timeline">{timeline}</ol>
    </section>
    <section id="quality" class="section final">
      <p class="eyebrow">Quality report</p>
      <h2>Previewable files were generated from a safe parser fallback.</h2>
      <p class="section-copy">Style direction: {style}. The raw Coder response is persisted for debugging without blocking tonight's website preview.</p>
      <a class="button primary" href="#hero">Back to top</a>
    </section>
  </main>
  <footer>Generated by Amarktai Builder fallback recovery.</footer>
  <script src="script.js"></script>
</body>
</html>
"""
    css = """*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:#05070b;color:#f8fafc;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}a{color:inherit}.topbar{position:sticky;top:0;z-index:10;display:flex;align-items:center;justify-content:space-between;padding:18px clamp(18px,5vw,72px);border-bottom:1px solid rgba(255,255,255,.1);background:rgba(5,7,11,.78);backdrop-filter:blur(16px)}.brand{text-decoration:none;font-weight:900;letter-spacing:.02em}.topbar nav{display:flex;gap:18px;color:#cbd5e1;font-size:14px}.topbar nav a{text-decoration:none}.hero{min-height:82vh;display:grid;grid-template-columns:minmax(0,1.05fr) minmax(300px,.95fr);gap:48px;align-items:center;padding:clamp(44px,8vw,118px) clamp(18px,7vw,96px);background:radial-gradient(circle at 18% 16%,rgba(0,230,118,.28),transparent 32%),radial-gradient(circle at 82% 18%,rgba(110,86,255,.24),transparent 34%),linear-gradient(145deg,#070a11,#080812 54%,#0d111c)}.eyebrow{margin:0 0 14px;color:#00e676;text-transform:uppercase;font-size:12px;letter-spacing:.16em;font-weight:900}.hero h1{max-width:880px;margin:0;font-size:clamp(44px,7vw,88px);line-height:.94;letter-spacing:0}.lede,.section-copy{max-width:780px;color:#cbd5e1;font-size:clamp(17px,2vw,22px);line-height:1.55}.actions{display:flex;gap:14px;flex-wrap:wrap;margin-top:28px}.button{display:inline-flex;align-items:center;justify-content:center;min-height:48px;padding:0 18px;border-radius:7px;text-decoration:none;font-weight:900}.primary{background:#00e676;color:#05100a}.secondary{border:1px solid rgba(255,255,255,.22);color:#e2e8f0;background:rgba(255,255,255,.06)}.preview{position:relative;min-height:430px;border:1px solid rgba(255,255,255,.12);border-radius:14px;background:linear-gradient(160deg,rgba(255,255,255,.12),rgba(255,255,255,.04));box-shadow:0 40px 120px rgba(0,0,0,.5);overflow:hidden}.preview:before{content:"";position:absolute;inset:-30%;background:conic-gradient(from 90deg,transparent,rgba(0,230,118,.35),transparent,rgba(123,92,255,.28),transparent);animation:spin 16s linear infinite}.preview-panel{position:absolute;border:1px solid rgba(255,255,255,.14);border-radius:12px;background:rgba(7,10,17,.82);box-shadow:0 18px 60px rgba(0,0,0,.32)}.large{inset:44px 36px 134px}.mid{left:76px;right:92px;bottom:72px;height:70px}.small{right:44px;top:132px;width:34%;height:94px}.section{padding:clamp(44px,7vw,96px) clamp(18px,7vw,96px);border-top:1px solid rgba(255,255,255,.09)}.section h2{max-width:900px;margin:0 0 16px;font-size:clamp(32px,4.5vw,58px);line-height:1.02}.cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-top:32px}.cards article,.timeline li{border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.055);border-radius:8px;padding:22px}.cards h3{margin:0 0 10px}.cards p{margin:0;color:#cbd5e1;line-height:1.55}.split{display:grid;grid-template-columns:minmax(0,.9fr) minmax(280px,1.1fr);gap:36px}.timeline{list-style:none;margin:0;padding:0;display:grid;gap:12px}.timeline li{display:flex;gap:16px;align-items:center}.timeline span{color:#00e676;font-weight:900}.final{background:linear-gradient(180deg,#05070b,#0d111c)}footer{padding:32px clamp(18px,7vw,96px);color:#94a3b8;border-top:1px solid rgba(255,255,255,.09)}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:860px){.hero,.split{grid-template-columns:1fr}.cards{grid-template-columns:1fr}.preview{min-height:300px}.topbar{align-items:flex-start;flex-direction:column}.topbar nav{flex-wrap:wrap}}@media(prefers-reduced-motion:reduce){*,*:before,*:after{animation:none!important;scroll-behavior:auto!important;transition:none!important}}"""
    js = """document.querySelectorAll('a[href^="#"]').forEach((link)=>{link.addEventListener('click',(event)=>{const target=document.querySelector(link.getAttribute('href'));if(target){event.preventDefault();target.scrollIntoView({behavior:'smooth',block:'start'});}});});document.documentElement.dataset.previewReady='true';"""
    report = f"""# Quality Report

Status: fallback files generated
...
Checks:
- Responsive viewport included
- Semantic sections included
- CTAs use valid internal anchors
- CSS/SVG-style visual fallback used
- Raw coder response persisted for debugging
"""
    readme = f"""# {brand}

This project was generated by Amarktai Builder's deterministic Coder recovery path.

## What happened

The Coder model returned output that could not be parsed into files. Rather than fail the build, Amarktai generated a premium static fallback so preview, validation, quality reporting, and retry/finalize flows can continue.

## Files

- `index.html` - production-style landing page
- `styles.css` - responsive cinematic styling
- `script.js` - small interaction layer
- `quality_report.md` - fallback quality evidence
...
"""
    return {
        "files": [
            ...
            {"path": "quality_report.md", "language": "markdown", "content": report},
...
def _quality_report_markdown(coverage: dict[str, Any], validation_state: dict[str, Any] | None = None) -> str:
    """Render a human-readable quality report file for dashboard visibility."""
    validation_state = validation_state or {}
    blockers = validation_state.get("errors") or validation_state.get("required_files_missing") or []
    warnings = validation_state.get("warnings") or []
    lines = [
        "# Quality Report",
        "",
        f"Coverage score: {coverage.get('coverageScore', coverage.get('score', 0))}",
        f"Preview available: {bool(coverage.get('previewUrl') or coverage.get('previewFallback') or validation_state.get('can_preview', True))}",
        f"Can finalize: {validation_state.get('can_finalize', validation_state.get('canFinalize', True))}",
        "",
        "## Checks",
        f"- Request satisfied: {coverage.get('requestSatisfied', coverage.get('request_satisfied', 'unknown'))}",
        f"- Files checked: {coverage.get('filesChecked', coverage.get('files_checked', 'unknown'))}",
        f"- Validation status: {validation_state.get('status', 'unknown')}",
    ]
    if blockers:
        lines.extend(["", "## Blockers", *[f"- {item}" for item in blockers[:10]]])
    if warnings:
        lines.extend(["", "## Warnings", *[f"- {item}" for item in warnings[:10]]])
    lines.extend(["", "Generated by Amarktai Builder."])
...
class Orchestrator:
    ...
    async def _persist_raw_agent_response(
        ...
        """Persist raw malformed model output for debugging without exposing secrets."""
        raw = raw_text or ""
        raw_stored = raw[:200_000]
        snippet = _safe_model_snippet(raw, 4000)
        payload = {
            "agent": agent,
            "model": model_label,
            "reason": reason,
            "raw_response": raw_stored,
            "raw_response_truncated": len(raw) > len(raw_stored),
        ...
        await self.db.projects.update_one(
            {"id": self.project_id},
            {"$set": {
                f"agent_raw_responses.{agent}": payload,
                "updated_at": _now(),
            }},
        )
        await self._record_event(
            agent,
            "warn",
            f"{agent.title()} output parsing failed; raw response captured for debugging.",
            meta={"model": model_label, "reason": reason, "raw_snippet": snippet},
        )
        await self._record_message(
            "system",
            agent,
            f"{agent.title()} output could not be parsed. Safe snippet:\n\n```text\n{snippet}\n```",
            meta={"model": model_label, "reason": reason, "raw_response_captured": True},
        )
    ...
    async def _run_agent_blocks(self, agent: str, system: str, user: str) -> dict:
        ...
        if data["files"]:
            ...
            return {"data": data, "model_label": model_label}
        ...
        if extracted_files:
            await self._record_event(
                ...
                meta={"model": model_label, "warnings": extract_warnings},
            )
            return {"data": {"files": extracted_files, "summary": extract_summary, "warnings": extract_warnings}, "model_label": model_label}
        ...
        try:
            ...
            try:
                ...
                if agent == "coder":
                    reason = f"Coder output could not be parsed or repaired: {repair_err}"
                    fallback = _deterministic_coder_fallback_files(user, reason)
                    await self._record_event(
                        agent,
                        "repaired",
                        "Coder fallback generated previewable static files after malformed output.",
                        meta={"model": model_label, "fallback_used": True},
                    )
                    await self._record_event(
                        agent,
                        "completed",
                        "Coder done (deterministic fallback files).",
                        meta={"model": model_label, "fallback_used": True},
                    )
                    return {"data": fallback, "model_label": model_label}
                err_msg = f"{agent.title()} output did not contain usable files or JSON after repair."
                await self._record_event(agent, "failed",
                                         f"Repair also failed: {repair_err}",
                                         meta={"repair_error": str(repair_err)})
                raise ValueError(err_msg) from repair_err
        if agent == "coder" and not data.get("files"):
            await self._persist_raw_agent_response(
                agent,
                raw,
                model_label,
                "Coder JSON parsed but did not contain usable files.",
            )
            fallback = _deterministic_coder_fallback_files(
                user,
                "Coder JSON parsed but did not contain usable files.",
            )
            await self._record_event(
                agent,
                "repaired",
                "Coder fallback generated previewable static files because parsed output contained no files.",
                meta={"model": model_label, "fallback_used": True},
            )
            await self._record_event(
                agent,
    ...
    async def _run_build_pipeline(self, user_prompt: str, mode: str,
        ...
        if not any(f.get("path") == "quality_report.md" for f in final_files):
            quality_markdown = _quality_report_markdown(coverage, validation_state)
            await self.fs.write("quality_report.md", quality_markdown, "markdown")
            await self.emit({"type": "file_written", "data": {"path": "quality_report.md"}})
            final_files = await self.fs.list_full()
