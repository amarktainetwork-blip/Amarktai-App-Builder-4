...
export default function CapabilityStatus({ compact = false }) {
  ...
  return (
    <section data-testid="capability-status" className="glass-panel overflow-hidden rounded-3xl">
      <div className="flex items-center justify-between border-b border-amk-line px-4 py-4">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-amk-accent">Capability truth</div>
          {!compact && <h2 className="mt-1 font-display text-2xl font-semibold tracking-tight text-white">Every capability is truth-gated.</h2>}
        </div>
        <button onClick={refresh} className="inline-flex h-9 items-center gap-2 rounded-2xl border border-amk-line px-3 font-mono text-[10px] uppercase tracking-wider text-amk-fg3 hover:bg-amk-surface hover:text-white">
      ...
      <div className={`grid gap-3 p-4 ${compact ? "sm:grid-cols-2 lg:grid-cols-3" : "md:grid-cols-2 xl:grid-cols-3"}`}>
        {CORE_CAPABILITIES.map(({ key, label }) => {
          const cap = data?.summary?.[key];
          const status = getStatus(cap, key);
          return (
            <div key={key} className="rounded-2xl border border-amk-line bg-amk-base/70 px-4 py-3">
      ...
      {!compact && (
        <>
          <div className="border-y border-amk-line bg-amk-base/70 px-4 py-3">
...
function GenxModelSummary({ runtime }) {
  ...
  return (
    <div className="border-b border-amk-line bg-amk-base/70 px-4 py-3">
      ...
      <div className="mt-2 flex flex-wrap gap-2 font-mono text-[10px] uppercase tracking-wider text-amk-fg2">
        <span className="rounded-full border border-amk-line px-2 py-1 text-amk-accent">{total} discovered models</span>
        {rows.map(([key, label]) => (
          <span key={key} className="rounded-full border border-amk-line px-2 py-1">{label}: {counts[key] || 0}</span>
        ))}
      </div>
    </div>
  );
