...
export default function CodeViewer({ projectId, path }) {
  ...
  if (!path) {
    return (
      <div data-testid="code-viewer-empty" className="grid-bg h-full grid place-items-center font-mono text-[11px] text-amk-fg3">
    ...
    <div data-testid="code-viewer" className="h-full flex flex-col">
      <div className="h-9 border-b border-amk-line bg-amk-base flex items-center justify-between px-3 shrink-0">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[11px] text-amk-fg">{path}</span>
          <span className="font-mono text-[10px] text-amk-fg3 uppercase">{lang}</span>
        </div>
        <button
